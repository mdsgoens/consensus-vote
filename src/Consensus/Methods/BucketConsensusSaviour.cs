...
{
    public sealed class BucketConsensusSaviour : BucketConsensusBase
    {
        public override ElectionResults GetElectionResults(CandidateComparerCollection<BucketBallot<Bucket>> ballots)
        {
            ...
            var approvalByBallot = new Dictionary<BucketBallot<Bucket>, ulong>();
            ...
            {
                var firstChoiceCandidates = ballot.Buckets.IndexesWhere(a => a == Bucket.Best).ToList();
            ...
            while (true)
            {
                ...
                var potentialSaviours = Enumerable.Range(0, ballots.CandidateCount).Select(s =>
                {
                    ...
                    var approvalsRequired = new List<BucketBallot<Bucket>>();

                    foreach (var (ballot, count) in ballots.Comparers)
                    {
                        var saviourRank = ballot.Buckets[s];

                        var lowestBogeymanRank = previousWinners
                            .Select(b => ballot.Buckets[b])
                            .Min();
                ...
                if (!potentialSaviours.Any())
                {
                    ...
                    foreach (var (ballot, count) in ballots.Comparers)
                    {
                        ...
                        if (lowestWinnerRank != Bucket.Bad)
                            continue;
                        ...
                        var newApprovals = ballot.Buckets
                ...
                else
                {
                        ...
                        .Select(gp => (gp.Key, gp.ToList())))
                    {
                        ...
                        approvalByBallot[ballot] = bestCoalition | GetCoalition(newApprovals);
                    }
                }
