...
{
    public sealed class BucketConsensusMututalMajority : BucketConsensusBase
    {
        ...
        IEnumerable<List<int>> GetAllSubsets(int candidateCount)
        {
            if (candidateCount == 1)
            {
            ...
            {
                foreach (var subset in GetAllSubsets(candidateCount - 1))
                {
                    yield return subset;
                    yield return subset.Append(candidateCount - 1).ToList();
                }
            }
        }
    }
